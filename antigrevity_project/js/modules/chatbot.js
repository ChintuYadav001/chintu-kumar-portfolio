...
import { openCookieModal } from './cookies.js';
...
export function initChatbot() {
  ...
  function generateSmartAnswer(query) {
    ...
    // ----------------------------------------------------------------------
    // 18. Cookies & Privacy Inquiries
    // ----------------------------------------------------------------------
    if (hasAny('cookie', 'cookies', 'privacy', 'tracking', 'save detail', 'save details', 'data privacy', 'visitor detail', 'visitor details')) {
      return {
        text: `**Cookie & Visitor Analytics System**\n\nChintu's portfolio uses browser cookies to:\n\n• Remember your **theme preferences** (dark/light mode) and **voice assistant state**\n• Assign an anonymous **Visitor ID** and count returning visits\n• Record traffic sources (e.g. LinkedIn, GitHub, Direct)\n• Remember the last section viewed so you resume where you left off\n\nAll cookies are stored securely inside your browser and never shared with third parties. You can inspect or clear your cookies at any time!`,
        actions: [
          { type: 'cookie-modal', label: '🍪 View My Cookie Details', icon: 'cookie' },
          { type: 'resume-modal', label: '📄 View ATS Resume', icon: 'fileText' }
        ]
      };
    }
    ...
    // ----------------------------------------------------------------------
    // 19. Conversational Greetings & Pleasantries
    // ----------------------------------------------------------------------
